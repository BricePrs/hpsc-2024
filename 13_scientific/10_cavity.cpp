...
int main() {
	...
	ofstream ufile("u.dat", std::ios::out | std::ios::trunc);
	ofstream vfile("v.dat", std::ios::out | std::ios::trunc);
	ofstream pfile("p.dat", std::ios::out | std::ios::trunc);
	...
	for (int n = 0; n < nt; n++) {
		for (int j = 1; j < ny - 1; j++) {
			for (int i = 1; i < nx - 1; i++) {
				b[j][i] = rho * (1.0 / dt * ((u[j][i+1] - u[j][i-1]) / (2.0 * dx) +
								 ...
								 ((u[j][i+1] - u[j][i-1]) / (2.0 * dx)) * ((u[j][i+1] - u[j][i-1]) / (2.0 * dx)) -
								 2.0 * ((u[j+1][i] - u[j-1][i]) / (2.0 * dy) *
										(v[j][i+1] - v[j][i-1]) / (2.0 * dx)) -
								 ((v[j+1][i] - v[j-1][i]) / (2.0 * dy)) * ((v[j+1][i] - v[j-1][i]) / (2.0 * dy)));
			}
		}
		...
		for (int it = 0; it < nit; it++) {
			...
			for (int j = 0; j < ny; j++) {
				p[j][0] = p[j][1];           // dp/dx = 0 at x = 0
				p[j][nx-1] = p[j][nx-2];     // dp/dx = 0 at x = 2
			}
			for (int i = 0; i < nx; i++) {
		...
		for (int i = 0; i < nx; i++) {
			u[0][i] = 0;
			u[ny-1][i] = 1;  // set velocity on cavity lid
			v[0][i] = 0;
			v[ny-1][i] = 0;
		}
